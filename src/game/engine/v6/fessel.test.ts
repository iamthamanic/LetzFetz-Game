...
describe('Fessel apply + start tick', () => {
  it('applies intensity to first component and decays on start restore', () => {
    const board = {
      technik: {
        instanceId: 't1',
    ...
    const applied = applyFesselToBoard(board, 2);
  ...
  it('Glutfessel TE applies Fessel to opponent after weak defense', () => {
    ...
    expect(next.players.p2.formula.technik?.fesselIntensity).toBe(1);
    expect(next.lastEvent ?? '').toMatch(/Fessel/);
  });

  it('defense 5–6 can zero Fessel intensity', () => {
    let state = createGame({
      pack: V6_CORE_PACK,
      p1CharacterId: V6_CORE_PACK.characters[0].id,
      p2CharacterId: V6_CORE_PACK.characters[1]?.id ?? V6_CORE_PACK.characters[0].id,
