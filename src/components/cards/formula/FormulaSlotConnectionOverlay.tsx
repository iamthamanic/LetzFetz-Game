/**
 * SVG overlay: flowing connections from filled Formelplätze into the combo preview.
 * Location: src/components/cards/formula/FormulaSlotConnectionOverlay.tsx
 */
...
} from './formulaSlotConnectionPaths';

const STROKE_BY_ROLE: Record<FormulaSlotRole, string> = {
  technik: '#34d399',
  essenz: '#38bdf8',
  katalysator: '#fbbf24',
};

const FLOW_DELAY_BY_ROLE: Record<FormulaSlotRole, string> = {
  technik: '0ms',
  essenz: '120ms',
  katalysator: '240ms',
};

interface FormulaSlotConnectionOverlayProps {
  /** Which roles currently have a filled slot (port anchors). */
  filledRoles: FormulaSlotRole[];
  containerRef: React.RefObject<HTMLElement | null>;
  previewTargetRef: React.RefObject<HTMLElement | null>;
  slotAnchorRefs: React.MutableRefObject<Partial<Record<FormulaSlotRole, HTMLElement | null>>>;
  testId?: string;
}
...
export function FormulaSlotConnectionOverlay({
  filledRoles,
  containerRef,
  previewTargetRef,
  slotAnchorRefs,
  testId = 'formula-slot-connections',
}: FormulaSlotConnectionOverlayProps) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const portEl =
        targetEl.querySelector<HTMLElement>('[data-testid="formula-preview-connection-port"]') ??
        targetEl.querySelector<HTMLElement>('[data-testid="build-preview-connection-port"]');
      const target = portEl
      ...
      for (const role of FORMULA_SLOT_ORDER) {
        if (!filledRoles.includes(role)) continue;
  ...
  }, [filledRoles.join('|'), containerRef, previewTargetRef, slotAnchorRefs]);
